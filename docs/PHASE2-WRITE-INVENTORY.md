...
| Update ticket | `updateTicket` | `superops_tickets_update` | write_visible | None when technician explicitly requested; Closed requires `lifecycle=close` | Resolve displayId/ticketId uniquely; capture status/priority/assignment | Re-read `getTicket`; compare intended fields | Set previous status/assignment | IMPLEMENTED / READY FOR LIVE VALIDATION | Technician, tech group, status, priority, resolution supported. Resolved is ordinary technician work. Closed is write_visible but requires an explicit human close instruction. |
| Create ticket note | `createTicketNote` (live RPMC; published `createNote` is not present on this tenant) | `superops_tickets_add_note` | write_low PRIVATE / write_visible PUBLIC | None when explicitly requested | Ticket exists | Note id present on `getTicketNoteList` | Notes are not deleted by this MCP | IMPLEMENTED / READY FOR LIVE VALIDATION | `addedBy` omitted so API-token identity is used. Live 0.2.1 `createNote` was rejected by SuperOps. |
...
| Resolve alerts | `resolveAlerts` | `superops_alerts_resolve` | write_visible | None when technician explicitly requested | Alert ids required | Boolean true + optional asset-scoped re-read | Cannot unresolve; stop class if verification fails | IMPLEMENTED / READY FOR LIVE VALIDATION | Changes monitoring/workflow state; does not interrupt the endpoint. Not disruptive. |
...
| Create KB article | `createKbArticle` | `superops_kb_articles_create` | write_low (DRAFT+technicians) / write_visible (PUBLISHED or requesters) | None when explicitly requested | Parent collection id; caller supplies HTML | Re-read name/status (GraphQL has no body) | Leave TEST DRAFT article | IMPLEMENTED / READY FOR LIVE VALIDATION | RPMC live: wrap fragment HTML in `html/body`, default `loginRequired=true`, technician share AllUsers+AllGroups. Fragment HTML with `loginRequired=false` ISE'd. |
...
| Run script on asset | `runScriptOnAsset` | `superops_scripts_execute` | classified by effect/target; unknown → disruptive | Elicitation when unauthorized disruptive/destructive; Rules B/C scoped grants pre-authorize in scope | Unique asset; script metadata page-1 lookup | `actionConfigId` present | Not reversible; stop class if unexpected impact | IMPLEMENTED / READY FOR LIVE VALIDATION | Existing script IDs only. No arbitrary script text. Classification follows consequence, not delete/remove/clear verbs. AI cannot lower classification. `SCRIPT_CONSEQUENCE_RAISE` can only raise. |
| Request / inspect / revoke authorization grant | local HMAC grant | `rpmc_authorization_request_grant` / `_inspect_grant` / `_revoke_grant` | write_low / read | Human elicitation to mint Rules B/C grants. Model cannot self-select B/C. | Proposed profile, task, explicit targets | N/A (no SuperOps mutation) | Revoke or restart | IMPLEMENTED / READY FOR LIVE VALIDATION | Opaque signed grant. Process-ephemeral. Does not rewrite classification. |
...
- Ticket assignment, status, priority, and resolution are on `superops_tickets_update`. Closed requires `lifecycle=close` after an explicit human close instruction.
...
schema validation → target resolution (fail closed) → pre-write capture → **effective** consequence classification → authorization (Rules A default, or scoped B/C grant, or per-action elicitation) → idempotency begin → SuperOps `mutate()` (no retry) → post-write read verification → sanitized audit (`registeredClassification` + `effectiveClassification`).

RPMC ticket lifecycle: delegated handling may set **Resolved**; **Closed** requires an explicit human close instruction (`lifecycle=close`). See [AUTHORIZATION-PROFILES.md](AUTHORIZATION-PROFILES.md).
