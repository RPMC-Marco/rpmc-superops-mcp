...
| investigate_asset alerts | getAlertsForAsset | assetId | AssetDetailsListInput | `{assetId,listInfo}` | asset alerts | section `unavailable` | none | alerts for that asset only | `alertFilter.query=getAlertsForAsset tenantScan=false` |
...
| investigate_client name | getClientList | name | is | string | unique client | `unsupported_filter` / `ambiguous` | none | exact name; duplicates fail | `resolution=name_condition_is` |
...
| investigate_client assets/tickets | getAssetList / getTicketList | client.name | is | string from client | bounded pages | section failed → partial | none | page 1 | `filterAttributes=client.name` |
